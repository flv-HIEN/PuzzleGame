...
class Game{
    ...
    constructor(difficultyLevel=1){
        ...
        this.blocks1=document.getElementsByClassName("puzzle_block1");
        this.init();
    }
    ...
    init(){//position each block in its proper position
        ...
        for(let y=0;y<this.rows;y++){
            for(let x=0;x<this.cols;x++){
                let blockIdx=x+y*this.cols;
                if(blockIdx+1>=this.count)break;
                let block=this.blocks1[blockIdx];
                this.positionBlockAtCoord1(blockIdx,x,y);
            }
        }
    ...
    positionBlockAtCoord1(blockIdx,x,y){//position the block at a certain coordinates
        let block1=this.blocks1[blockIdx];
        block1.style.left=(x*block1.clientWidth)+"px";
        block1.style.top=(y*block1.clientWidth)+"px";
